...
public class PlayLocalMediaInstance {
    ...
    public void setData(CzMusicBean czMusicBean) {
        ...
        if (czMusicBean.getUseheadset().equals(com.xilu.wybz.common.MyCommon.TAG_RECORD_IN)) {
            //插上耳机录的，需要播放双曲
            File file = new File(FileUtils.getLocalplayurl(czMusicBean.getId()));
            if (file.exists()) {
                startMediaPlay(FileUtils.getLocalplayurl(czMusicBean.getId()), FileUtils.getMusicCachePath(com.xilu.wybz.common.MyCommon.TYPE_MAKE + czMusicBean.getId()));
            } else {
                startMediaPlay(FileUtils.getMusicCachePath(com.xilu.wybz.common.MyCommon.TYPE_MAKE + czMusicBean.getId()));
            }
        } else {
